<template>
  <b-container >
    ...
    <div style="text-align: center">
      <b-button variant="outline"> <b-icon icon="arrow-left" @click="$refs.carousel.prev()"/> </b-button>
      <b-button variant="outline"> <b-icon icon="arrow-right" @click="$refs.carousel.next()"/> </b-button>
    </div>
    </b-modal>
...
export default {
  ...
  data: () => ({
    messages: [
      {direction: 0, text:"Hallo! Ich bin Bruno, ein Chatbot! 🤖", time:"00:00"},
      {direction: 0, text:"Ich kann dir Fragen rund um die Nationalratswahlen 🗳 in Österreich beantworten. Im Moment bin ich noch in Entwicklung 🛠 und brauche eventuell einen Moment ⌛️ länger um deine Fragen zu beantworten.", time:"00:00"},
      {direction: 0, text:"Mein Entwickler möchte mit dir und mir eine Studie durchführen und herausfinden, wie gut wir zwei uns verstehen. Deswegen werde ich dir Anweisungen geben und dich durch diese Studie führen. Zu allererst öffne bitte folgende Seite und starte mit einer kleinen Umfrage: https://forms.gle/bF63LetxXuirH6dF9", time:"00:00"},
      {direction: 0, text:"Um mit mir zu sprechen, schick mir eine Sprachnachricht 🔊. Klick dafür auf den Button 🎙 unten, halte ihn solange du sprichst, und lass ihn los um die Frage abzuschicken.", time:"00:00"},
      {direction: 0, text:"Probieren wir das gleich aus:", time:"00:00"},
      {direction: 0, text:"Begrüß' mich!", time:"00:00"},
    ],
    slide: 0,
  ...
  methods:{
    ...
    getMessage(){
      this.messages.push({direction: 0, text:"New Message", time: this.getTimeString() })
    },
